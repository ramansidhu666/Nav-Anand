...
namespace Property.Admin
{
    public partial class NewsLetterList : System.Web.UI.Page
    {
        ...
        {
            DataTable dt = new DataTable();
            string str = "select image,second_Image from [Newsletter] where NewsLetterId=" + id;
            SqlDataAdapter adp = new SqlDataAdapter(str, conn);
            adp.Fill(dt);

            foreach (DataRow row in dt.Rows)
            {
                var Firstimage = row["Image"].ToString();
                if(Firstimage!=null && Firstimage != "")
                {
                    DeleteImage(Firstimage);
                }

                var secondimage = row["second_Image"].ToString();
        ...
        public void DeleteImage(string filePath)
        {
            try
            {
                ...
                var subPath = HttpContext.Current.Server.MapPath("~/uploadfiles");
